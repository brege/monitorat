class SpeedtestTable {
  ...
  addSourceMetadataField () {
    ...
    const hasSourceField = existingFields.some(f =>
      ...
      const results = await Promise.all(
        sources.map(async (source) => {
          try {
            ...
            const response = await fetch(`api/speedtest-${source}/history?${searchParameters.toString()}`, { cache: 'no-store' })
            if (!response.ok) {
              console.warn(`Failed to fetch speedtest from ${source}: HTTP ${response.status}`)
              return []
            }
            const payload = await response.json()
            return (payload.entries || []).map(entry => ({ ...entry, _source: source }))
          } catch (error) {
            console.warn(`Failed to fetch speedtest from ${source}:`, error.message)
            return []
          }
        })
      )
