class SpeedtestControls {
  ...
  setupEventListeners () {
    ...
    timeSeriesHandler.setupPeriodSelect(periodSelect, this.widget.config.chart.periods, this.widget.selectedPeriod, (period) => {
  ...
  async runSpeedtest () {
    ...
    try {
      const response = await fetch(`${this.widget.getApiBase()}/run`, { method: 'POST' })
      ...
      if (status) {
        ...
        const downloadMetric = this.widget.metricFields.find(metric => metric.field === 'download') || {}
        const uploadMetric = this.widget.metricFields.find(metric => metric.field === 'upload') || {}
        const pingMetric = this.widget.metricFields.find(metric => metric.field === 'ping') || {}
