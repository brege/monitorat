...
class ServicesModal {
  ...
  getServiceStatusInfo (service) {
    ...
    const checks = [
      ...(service.containers || []).map(container => ({ name: container, type: 'container' })),
      ...(service.services || []).map(serviceName => ({ name: serviceName, type: 'service' })),
      ...(service.timers || []).map(timer => ({ name: timer, type: 'timer' }))
    ]
