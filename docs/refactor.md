...
- [x] Audit tile layout math in `static/app.js` and `themes/default.css` to 
...
- [x] Move per-widget `--widget-min-width` defaults into each widget’s 
  `default.yaml`.
- [x] Read the per-widget min width from config and apply it as a CSS custom
  property on the widget root.
- [x] Keep CSS theme overrides valid so themes can still override min width if
  needed.
